...
namespace Geometry
{
	enum Color // Enumiration - Это перечисление, набор именнованных констант типа 'int'
	{
		red = 0x000000FF,
		green = 0x0000FF00,
		blue = 0x00FF0000,
...
#define SHAPE_GIVE_PARAMETERS start_x, start_y, line_width, color
	...
	public:
		...
		void draw()const override
		{
			//1) Полчаем окно консоли:
			HWND hwnd = GetConsoleWindow(); //HWND - Handler to Window обработчик окна
			//К переменной можно обратиться по имени, к окну можно обратится через HWND
			//  GetConsoleWindow() - возвращает hwnd окна консоли из текущего потока (команд)
			//2) Создаем контекст устройства:
			HDC hdc = GetDC(hwnd);//hdc - это контекст устройства
			//GetDC(hwnd) - Возвращает контекст устройства заданного окна
			...
			//5) Рисуем фигуру:
			::Rectangle(hdc, start_x, start_y,start_x + side_a, side_b);
